...
- **Live content:** a daily GitHub Action (`.github/workflows/refresh-feed.yml`) fetches the real Livewire RSS into `data/feed.json`; `assets/feed.js` folds items into the home Latest lists and per-topic "Latest wires" rows.
...
- **Universal search ↔ scans:** ⌘K palette searches the full content index and offers "⌁ Scan every wire for …" on scannable asks.
